...
void pack_matrix(matrix* input, int elem, matrix output, int width, int height)
{
  // first pack
  ...
  for (int row = 0; row < height; row++) // row
  {
    ...
    {
      output[row][col] = (input[0][row][col] << shift);
    }
  }
 
  for ( int i = 2; i <= elem; i++ )
  {
    shift = __M*(elem - i); // update shift e = 2... elem
    for (int row = 0; row < height; row++) // row
    {
      for (int col = 0; col < width; col++) // col
      {
        output[row][col] += ( input[i-1][row][col] << shift );
      }
    }
  }  
}
  ...
  for (int row = 0; row < height; row++) // row
  {
    for (int col = 0; col < width; col++) // col
    {
      for (int e = 1; e <= elem; e++)  // elem packed
      {
        ...
        bx = ( input[row][col] >> shift_down ) & __mask;

        output[e-1][row][col] += (bx << bitplane);
      }
    }
  }
}
...
void unpack_matrix_and_store_i(matrix input, matrix* output, int elem, int width, int height /*, int bitplane, int bits */)
{
  //int shift = bitplane - bits + 1;
    my_t bx;
  for (int row = 0; row < height; row++) // row
  {
    for (int col = 0; col < width; col++) // col
    {
      for (int e = 1; e <= elem; e++)  // elem packed
      {
        ...
        output[e-1][row][col] = bx; //+= (bx << shift);
      }
    }
  }
}
